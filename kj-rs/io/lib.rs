...
pub trait AsyncInputStream {
    ...
    fn pump_to<T: AsyncOutputStream>(
        &mut self,
        output: &mut T,
        amount: u64,
        ...
        Self: Sized,
    {
        async move {
            ...
            unoptimized_pump_to(self, output, amount, 0).await
        }
    }
    ...
    fn register_ancillary_message_handler<F>(&mut self, _handler: F) -> Result<()>
    where
        F: FnMut(&[AncillaryMessage]) + Send + 'static,
    {
    ...
    fn try_tee(&mut self, _limit: u64) {
...
pub trait AsyncOutputStream {
    ...
    fn write(&mut self, buffer: &[u8]) -> impl Future<Output = Result<()>>;
    ...
    fn write_vectored(&mut self, pieces: &[&[u8]]) -> impl Future<Output = Result<()>>;
    ...
    ///
    /// The default implementation always returns None.
    fn try_pump_from<T: AsyncInputStream>(
        &mut self,
        _input: &mut T,
        _amount: u64,
    ) -> impl Future<Output = Option<Result<u64>>> {
        async move { None }
    }
    ...
    fn when_write_disconnected(&mut self) -> impl Future<Output = Result<()>>;
}
...
impl AncillaryMessage {
    ...
    /// Interpret the ancillary message as the given struct type.
    ///
    /// Most ancillary messages are some sort of struct, so this is a convenient way to access it.
    /// Returns None if the message is smaller than the struct -- this can happen if the message
    ...
    /// Interpret the ancillary message as an array of items.
    ///
    /// If the message size does not evenly divide into elements of type T, the remainder is
    /// discarded -- this can happen if the message was truncated due to insufficient ancillary
    /// buffer space.
    #[must_use]
    pub fn as_array<T>(&self) -> &[T] {
        ...
        let count = self.data.len() / element_size;
        unsafe { std::slice::from_raw_parts(self.data.as_ptr().cast::<T>(), count) }
    }
}
...
pub async fn unoptimized_pump_to<I: AsyncInputStream, O: AsyncOutputStream>(
    ...
    while remaining > 0 {
        ...
        total_pumped += bytes_read as u64;
...
pub struct AsyncWriteAdapter<T> {
    inner: T,
}
...
impl<T: AsyncInputStream + Unpin> futures::io::AsyncRead for AsyncReadAdapter<T> {
    fn poll_read(
        ...
        let future = self.inner.try_read(buf, min_bytes);
...
impl<T: AsyncOutputStream + Unpin> futures::io::AsyncWrite for AsyncWriteAdapter<T> {
    fn poll_write(
        ...
        let future = self.inner.write(buf);
        let mut pinned_future = Box::pin(future);
...
pub mod ffi {
    ...
    unsafe extern "C++" {
        ...
        #[must_use]
        fn try_get_length(self: Pin<&mut CxxAsyncInputStream>) -> u64; // Return 0 if unknown
        ...
        #[must_use]
        fn try_get_length(self: Pin<&mut CxxAsyncIoStream>) -> u64;
...
pub struct CxxAsyncInputStream {
    inner: std::pin::Pin<Box<ffi::CxxAsyncInputStream>>,
}

impl CxxAsyncInputStream {
    /// Create a new `CxxAsyncInputStream` from the FFI type
    pub fn new(ffi_stream: ffi::CxxAsyncInputStream) -> Self {
        Self {
...
impl AsyncInputStream for CxxAsyncInputStream {
    ...
    async fn pump_to<T: AsyncOutputStream>(&mut self, output: &mut T, amount: u64) -> Result<u64>
    where
        Self: Sized,
    {
...
impl AsyncOutputStream for CxxAsyncOutputStream {
    ...
    async fn try_pump_from<T: AsyncInputStream>(
        &mut self,
        _input: &mut T,
        _amount: u64,
    ) -> Option<Result<u64>> {
