...
pub fn take_maybe_own_ret(
    val: Maybe<Own<OpaqueCxxClass>, Niche>,
) -> Maybe<Own<OpaqueCxxClass>, Niche> {
...
pub fn take_maybe_own(val: Maybe<Own<OpaqueCxxClass>, Niche>) {
...
pub unsafe fn take_maybe_ref_ret<'a>(val: Maybe<&'a u64, Niche>) -> Maybe<&'a u64, Niche> {
...
pub fn take_maybe_ref(val: Maybe<&u64, Niche>) {
...
pub mod tests {
    ...
    #[test]
    fn test_none_ptr() {
        let maybe = ffi::return_maybe_ptr_none();
        assert!(maybe.is_none());
        assert!(!maybe.is_some());
    }

    #[test]
    fn test_none_ptr_opt() {
        let maybe = ffi::return_maybe_ptr_none();
        let maybe: Option<*const i64> = maybe.into();
        assert!(maybe.is_none());
    }
