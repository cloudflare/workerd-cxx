...
kj::Maybe<int64_t const*> return_maybe_ptr_none() {
  kj::Maybe<int64_t const*> ret = kj::Maybe<int64_t const*>();
  return kj::mv(ret);
}
...
kj::Maybe<int64_t const*> return_maybe_ptr_some() {
  kj::Maybe<int64_t const*> ret = kj::some(&var);
  return kj::mv(ret);
}
...
kj::Maybe<kj::Own<OpaqueCxxClass>> return_maybe_own_some() {
  kj::Own<OpaqueCxxClass> ret = kj::heap<OpaqueCxxClass>(14);
  kj::Maybe<kj::Own<OpaqueCxxClass>> maybe = kj::some(kj::mv(ret));
  return kj::mv(maybe);
}
